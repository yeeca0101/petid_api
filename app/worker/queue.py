...
class QueueWorker:
    ...
    def run_once(self) -> QueueWorkerResult:
        ...
        claim = self._claim_next_job()
        if claim is None:
            return QueueWorkerResult(handled_job=False, reclaimed_count=reclaimed_count)

        job_id, job_type, payload = claim
        handler = self.handlers.get(job_type)
        if handler is None:
            self._fail_without_handler(job_id=job_id, job_type=job_type)
        ...
        self._mark_running(job_id=job_id)
        stop_event = threading.Event()
        heartbeat_thread = threading.Thread(
            target=self._heartbeat_loop,
            kwargs={"job_id": job_id, "stop_event": stop_event},
            daemon=True,
        )
        ...
        try:
            ...
            logger.exception("Queue job failed | worker_id=%s | job_id=%s | job_type=%s", self.config.worker_id, job_id, job_type)
            self._mark_failed(
                job_id=job_id,
            ...
            self._mark_succeeded(job_id=job_id, result=result)
        finally:
            stop_event.set()
    ...
    def _claim_next_job(self) -> tuple[uuid.UUID, str, dict[str, Any]] | None:
        with self.db.session_scope() as session:
            repo = ReIdRepository(session)
            job = repo.claim_next_job(worker_id=self.config.worker_id)
            if job is None:
                return None
            repo.append_job_event(
                job_id=job.job_id,
            ...
            return job.job_id, job.job_type, dict(job.payload)

    def _mark_running(self, *, job_id: uuid.UUID) -> None:
        with self.db.session_scope() as session:
            repo = ReIdRepository(session)
            repo.start_job_run(job_id, worker_id=self.config.worker_id)
            repo.append_job_event(
                job_id=job_id,
    ...
    def _mark_succeeded(self, *, job_id: uuid.UUID, result: dict[str, Any]) -> None:
        with self.db.session_scope() as session:
            repo = ReIdRepository(session)
    ...
    def _mark_failed(self, *, job_id: uuid.UUID, error_code: str, error_message: str) -> None:
        with self.db.session_scope() as session:
            repo = ReIdRepository(session)
    ...
    def _fail_without_handler(self, *, job_id: uuid.UUID, job_type: str) -> None:
        self._mark_failed(
            job_id=job_id,
    ...
    def _heartbeat_loop(self, *, job_id: uuid.UUID, stop_event: threading.Event) -> None:
        while not stop_event.wait(self.config.heartbeat_interval_s):
            try:
                ...
                logger.exception("Queue heartbeat update failed | worker_id=%s | job_id=%s", self.config.worker_id, job_id)
